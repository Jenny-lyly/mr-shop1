...
public class PayServiceImpl extends BaseApiService implements PayService {
    ...
    public void requestPay(PayInfoDTO payInfoDTO, String token, HttpServletResponse response) {
        ...
        if(orderResult.getCode() == 200){
            ...
            List<String> titleList = orderInfo.getOrderDetailList().stream().map(orderDetail -> {
...
//            out.println("fail");
    ...
    @Override
    public void returnHTML(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(signVerified) {
            ...
            try {
                ...
                log.debug("trade_no:{},out_trade_no:{},total_amount:{}",trade_no,out_trade_no,total_amount);
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
        }else {
